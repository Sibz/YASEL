...
namespace YASEL
{
    partial class Program
    {
        ...
        //# Requires WpInfo
        //# Requires VarStore
        //# Requires NavSettings
        //# Requires BlockFunctions
        //# Requires GyroFunctions
        //# Requires ThrusterFunctions
        ...
        public class NavManager
        {
            ...
            public NavManager(NavSettings s = null)
            {
                ...
                dbug("Initialising NavManager", 2);


                runCount = 0;
                // Setup GPS Blocks
                leftBlock = GetBlock(settings.LeftBlockName);
                rightBlock = GetBlock(settings.RightBlockName);
                topBlock = GetBlock(settings.TopBlockName);

                bottomBlock = GetBlock(settings.BottomBlockName);
                rearBlock = GetBlock(settings.RearBlockName);
                fwdBlock = GetBlock(settings.FwdBlockName);

                if (leftBlock == null || rightBlock == null || topBlock == null || bottomBlock == null || rearBlock == null || fwdBlock == null) { dbug(ERROR1); throw new Exception(ERROR1); }
                //Setup block groups
                gyros = GetBlockGrp(settings.GyroGroupName); if (gyros.Count == 0) { dbug(ERROR2); throw new Exception(ERROR2); }
                fwdThrusters = GetBlockGrp(settings.FwdThrustGroupName); if (fwdThrusters.Count == 0) { dbug(ERROR3); throw new Exception(ERROR3); }
                rvsThrusters = GetBlockGrp(settings.RvsThrustGroupName); if (rvsThrusters.Count == 0) { dbug(ERROR3); throw new Exception(ERROR3); }
                leftThrusters = GetBlockGrp(settings.LeftThrustGroupName); if (leftThrusters.Count == 0) { dbug(ERROR3); throw new Exception(ERROR3); }
                rightThrusters = GetBlockGrp(settings.RightThrustGroupName); if (rightThrusters.Count == 0) { dbug(ERROR3); throw new Exception(ERROR3); }
                upThrusters = GetBlockGrp(settings.UpThrustGroupName); if (upThrusters.Count == 0) { dbug(ERROR3); throw new Exception(ERROR3); }
                downThrusters = GetBlockGrp(settings.DownThrustGroupName); if (downThrusters.Count == 0) { dbug(ERROR3); throw new Exception(ERROR3); }

                // Setup Controller Block (for dampener control)
                if (settings.CockpitName == null)
                {
                    var controllers = new List<IMyTerminalBlock>();
                    gts.GetBlocksOfType<IMyShipController>(controllers, BelongsToGrid);
                ...
                {
                    controller = GetBlock(settings.CockpitName);
                }
                if (controller == null) { dbug(ERROR4); throw new Exception(ERROR4); }
            ...
            public void Tick()
            {
                ...
                else if (runCount == 8)
                {
                    ...
                    runCount++;
                dbug("Tick - Done routine stuff, doing WP Stuff", 2);
            ...
            {
                if (InStrI(wi.type, "wp") || InStrI(wi.type, "travel"))
                    wi.TravelTo();
                if (InStrI(wi.type, "align"))
                    wi.AlignTo();
                if (InStrI(wi.type, "level"))
                    wi.LevelTo();
                if (InStrI(wi.type, "wait"))
                    wi.WaitFor();
                if (InStrI(wi.type, "center"))
                    wi.CenterOn();
                ...
                if (!waypoints.ContainsKey(waypoints.Count))
                    ...
                    dbug("Unable to add waypoint - key exists\n\t" + wi.GetLCDString());
            }
            ...
            public void Pause()
            {
                YawStop(gyros);
                PitchStop(gyros);
                GyroOverride(gyros, false);
                TurnOnOffDampeners(controller, true);
                SetThrustOverride(fwdThrusters, 0);
                SetThrustOverride(rvsThrusters, 0);
                SetThrustOverride(leftThrusters, 0);
                SetThrustOverride(rightThrusters, 0);
                SetThrustOverride(upThrusters, 0);
                SetThrustOverride(downThrusters, 0);
                paused = true;
            ...
            private bool center(WpInfo wi = null)
            {
                ...
                SetThrustOverride(leftThrusters, lThrust);
                SetThrustOverride(rightThrusters, rThrust);
                SetThrustOverride(upThrusters, uThrust);
                SetThrustOverride(downThrusters, dThrust);
                SetThrustOverride(fwdThrusters, fwdThrust);
                SetThrustOverride(rvsThrusters, rvsThrust);
            ...
            private bool level(WpInfo wi = null)
            {
                ...
                if (dotC > settings.AlignMargin)
                    RollL(gyros, rollSpeed);
                else if (dotC < -settings.AlignMargin)
                    RollR(gyros, rollSpeed);
                else if (dotC != 0)
                {
                    RollStop(gyros);
                    GyroOverride(gyros, false);
            ...
            private bool align(WpInfo wi = null, bool dampening = true)
            {
                ...
                GyroOverride(gyros);
                if (dampening) TurnOnOffDampeners(controller, true);
                ...
                if (dotU > settings.AlignMargin)
                    PitchD(gyros, pitchSpeed);
                if (dotU < 0 - settings.AlignMargin)
                    PitchU(gyros, pitchSpeed);
                ...
                { YawStop(gyros); PitchStop(gyros); GyroOverride(gyros, false); return true; }
            ...
            private bool travel(WpInfo wi = null)
            {
                ...
                { arrived = true; YawStop(gyros); PitchStop(gyros); RollStop(gyros); GyroOverride(gyros, false); if (wi.noStop)dampening = false; }
                SetThrustOverride(thrusters, fwdThrust);
                SetThrustOverride(inverseThrusters, rvsThrust);
                TurnOnOffDampeners(controller, dampening);
                return arrived;
            }
